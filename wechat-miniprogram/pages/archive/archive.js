...
function buildArchive(briefData) {
  return (briefData.archive || []).map((item) => {
...
Page({
  ...
  loadBriefData() {
    briefStore.getBriefData().then((briefData) => {
      const archive = buildArchive(briefData);
      const selectedIssue = archive.find((item) => item.date === this.data.selectedDate) || archive[0] || null;
